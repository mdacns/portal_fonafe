...
def limpiar_dataframe_power_pivot(df):
  if df is None or df.empty:
    return pd.DataFrame()
  df = df.loc[
      :, ~df.columns.astype(str).str.contains("Unnamed", case=False, na=False)
  ]
...
if "df_global_stock" not in st.session_state:
  url_stock = (
      f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&gid={GID_STOCK}"
  )
  fallback_stock = {
      "Id Item": [1, 2, 3, 4],
      "Fec. Ingreso": ["2026-01-10", "2026-01-12", "2026-02-01", "2026-02-05"],
      "Item": ["Laptop", "Monitor", "Docking", "Teclado"],
      "Marca": ["Lenovo", "Dell", "HP", "Logitech"],
      "Part number": ["21TCS1DM00", "DEL-24", "HP-DC", "K-100"],
      "Tipo Item": ["Hardware", "Periférico", "Accesorio", "Periférico"],
      "Descripción": [
          "Laptop Lenovo ThinkPad T14s Gen6 16 gb",
          "Monitor 24 pulg",
          "USB-C Dock",
          "Kit Teclado Mouse",
      ],
      ...
      "RMA": ["N/A", "RMA-001", "N/A", "N/A"],
      "SN CAMBIO": ["N/A", "N/A", "SN-CAMB-01", "N/A"],
      "GR SONDA": ["N/A", "N/A", "N/A", "N/A"],
      "GR FISICA": ["N/A", "N/A", "N/A", "N/A"],
      "COURIER": ["N/A", "N/A", "N/A", "N/A"],
      "TICKET ARANDA": ["N/A", "N/A", "N/A", "N/A"],
...
if st.session_state.seccion_actual == "Home":
  ...
  st.markdown(
      "<p style='color: #475569; font-size: 0.95rem; margin-bottom: 20px; font-weight:"
      " 600;'>Seleccione el módulo al que desea acceder:</p>",
      unsafe_allow_html=True,
  )
  ...
  with col_h1:
    st.markdown(
        ...
        " 0;'>📦 Módulo de Inventario Maestro (Modelo Relacional)</p>",
        unsafe_allow_html=True,
    )
  with col_h2:
    ...
    with sub_c1:
      if st.button(
          "🗑️ Limpiar", key="btn_limpiar_cols", use_container_width=True
      ):
        st.session_state.cols_inv_maestro = []
        st.rerun()
    with sub_c2:
      if st.button(
          "⬅️ Volver al Menú", key="btn_inv_home", use_container_width=True
      ):
        cambiar_seccion("Home")

  st.markdown("<hr style='margin: 4px 0 10px 0;'>", unsafe_allow_html=True)

  df_inv = limpiar_dataframe_power_pivot(
      st.session_state.df_inventario_sedes.copy()
  )
  df_ent = limpiar_dataframe_power_pivot(
      st.session_state.df_entidades_ruc.copy()
  )
  ...
  if col_ent_inv and col_ent_ent:
    ...
    df_inv_temp["_key_join"] = (
        df_inv_temp[col_ent_inv].astype(str).str.strip().str.lower()
    )
    df_ent_temp["_key_join"] = (
        df_ent_temp[col_ent_ent].astype(str).str.strip().str.lower()
    )

    cols_a_traer = [
        c
        for c in df_ent_temp.columns
        if c not in [col_ent_ent, "_key_join"] and c not in df_inv_temp.columns
    ]
  ...
  if not df_stk.empty:
    ...
    for c_req in [
      ...
      if (
          c_encontrada
          and c_encontrada not in cols_stock_requeridas
          and c_encontrada not in df_master.columns
      ):
  ...
  st.markdown(
      "<p style='font-size: 11px; font-weight: 700; color: #475569;"
      " margin-bottom: 4px;'>Filtros en Cascada (Modelo de Datos)</p>",
      unsafe_allow_html=True,
  )
  ...
  with f1:
    if c_ent:
      entidades_disp = ["Todas"] + sorted([
          str(x)
          for x in df_master[c_ent].dropna().unique()
          if str(x) != ""
      ])
      sel_entidad = st.selectbox(
          "1. Entidad", entidades_disp, key="cascada_entidad"
      )
  ...
  with f2:
    if c_reg:
      regiones_disp = ["Todas"] + sorted([
          str(x)
          for x in df_master[c_reg].dropna().unique()
          if str(x) != ""
      ])
      sel_region = st.selectbox(
          "2. Región", regiones_disp, key="cascada_region"
      )
      if sel_region != "Todas":
  ...
  with f3:
    if c_prov:
      provincias_disp = ["Todas"] + sorted([
          str(x)
          for x in df_master[c_prov].dropna().unique()
          if str(x) != ""
      ])
      sel_provincia = st.selectbox(
          "3. Provincia", provincias_disp, key="cascada_provincia"
      )
  ...
  with f4:
    if c_dist:
      distritos_disp = ["Todas"] + sorted([
          str(x)
          for x in df_master[c_dist].dropna().unique()
          if str(x) != ""
      ])
      sel_distrito = st.selectbox(
          "4. Distrito", distritos_disp, key="cascada_distrito"
      )
  ...
  todas_columnas = [
      c for c in df_master.columns.tolist() if c.lower() not in ["serie", "marca"]
  ]
  ...
  st.markdown(
      "<p style='font-size: 11px; font-weight: 700; color: #475569;"
      " margin-bottom: 6px;'>Seleccione las columnas a mostrar (Cápsulas"
      " compactas):</p>",
      unsafe_allow_html=True,
  )
  ...
  if not st.session_state.cols_inv_maestro:
    st.info(
        "ℹ️ Aún no ha seleccionado ninguna columna. Por favor, haga clic en"
        " las cápsulas superiores para elegir las columnas que desea"
        " visualizar."
    )
    ...
    if (
        not df_final_mostrar.empty
        and str(df_final_mostrar.iloc[0, 0]).strip()
        == str(df_final_mostrar.columns[0]).strip()
    ):
    ...
    st.markdown(
        f"<p style='font-size: 11px; color: #64748b; font-weight: 500;"
        f" margin-bottom: 6px;'>Mostrando <b>{len(df_final_mostrar)}</b>"
        f" registros coincidentes y"
        f" <b>{len(st.session_state.cols_inv_maestro)}</b> columnas"
        " seleccionadas.</p>",
        unsafe_allow_html=True,
    )
  ...
  with col_h1:
    st.markdown(
        "<p style='font-size: 1.15rem; font-weight: 700; color: #0f172a; margin:"
        " 0;'>📊 Control de Stock e Inventario General</p>",
        unsafe_allow_html=True,
    )
  with col_h2:
  ...
  if df_stock.empty:
    ...
    with col_busq_p1:
      busqueda_general_stock = st.text_input(
          "🔍 Búsqueda rápida",
          "",
          key="filtro_busq_gen_stock",
    ...
    if busqueda_general_stock:
      mask_gen = df_f.apply(
          ...
          .str.contains(busqueda_general_stock, case=False, na=False)
          .any(),
          axis=1,
      )
      df_f = df_f[mask_gen]
    ...
    with st.expander("⚙️ Personalizar columnas visibles", expanded=False):
      ...
      for i, col_name in enumerate(cols_todas):
        with exp_cols[i % 4]:
          ocultar = st.checkbox(
              f"{col_name}",
              value=col_name not in st.session_state.stock_cols_ocultas,
              key=f"stock_show_{col_name}",
          )
          if (
              not ocultar
              and col_name not in st.session_state.stock_cols_ocultas
          ):
    ...
    cols_visibles = [
        c for c in cols_todas if c not in st.session_state.stock_cols_ocultas
    ]
    ...
    if not cols_visibles:
      ...
      st.info(
          "ℹ️ No se encontraron registros con la combinación de filtros"
          " seleccionados."
      )
    else:
      ...
      st.markdown(
          f"<p style='font-size: 11px; color: #64748b; font-weight: 500;"
          f" margin-bottom: 6px;'>Mostrando <b>{len(df_f)}</b> registros"
          f" filtrados y <b>{len(cols_visibles)}</b> columnas activas.</p>",
          unsafe_allow_html=True,
      )
  ...
  with col_h1:
    st.markdown(
        "<p style='font-size: 1.15rem; font-weight: 700; color: #0f172a; margin:"
        " 0;'>🛠️ Gestión de averías y cambios</p>",
        unsafe_allow_html=True,
    )
  with col_h2:
  ...
  if (
      "mensaje_exito_cambio" in st.session_state
      and st.session_state.mensaje_exito_cambio
  ):
  ...
  if serie_a_validar:
    if df_averias.empty:
      ...
      df_averias["Serie_clean"] = (
          df_averias["Serie"].astype(str).str.strip()
      )
      busqueda_clean = serie_a_validar.strip()

      match_df = df_averias[
          df_averias["Serie_clean"].str.lower() == busqueda_clean.lower()
      ]

      if not match_df.empty:
        st.info(
            f"ℹ️ Se encontró coincidencia para la serie:"
            f" **{serie_a_validar}**"
        )
        ...
        st.markdown(
            "<hr style='margin: 2px 0 6px 0;'>", unsafe_allow_html=True
        )
        st.markdown(
            "<p style='font-weight: 600; color: #1e293b; margin-bottom:"
            " 2px;'>¿Deseas cambiar el equipo?</p>",
            unsafe_allow_html=True,
        )
        ...
        if opcion_cambio:
          part_number_buscado = str(
              match_df["Part number"].values[0]
          ).strip()

          df_averias["Part_number_clean"] = (
              df_averias["Part number"].astype(str).str.strip()
          )
          df_averias["Entidad_clean"] = (
              df_averias["Entidad"].astype(str).str.strip()
          )

          df_filtrado_backup = df_averias[
              (
                  df_averias["Part_number_clean"].str.lower()
                  == part_number_buscado.lower()
              )
              & (
                  df_averias["Entidad_clean"].str.lower()
                  == opcion_cambio.lower()
              )
          ]
          ...
          if not df_filtrado_backup.empty:
            ...
            campos_respaldo_disp = [
                c for c in campos_respaldo if c in df_filtrado_backup.columns
            ]
            ...
            filas_seleccionadas = edited_backup_df[
                edited_backup_df["Seleccionar"] == True
            ]
            ...
            if not filas_seleccionadas.empty:
              ...
              if st.button(
                  "🚀 Proceder con el cambio", key="btn_proceder_cambio"
              ):
                idx_buscado = match_df.index[0]
                st.session_state.df_global_stock.loc[
                    idx_buscado, "Tipo estado"
                ] = "Averiado"
                st.session_state.df_global_stock.loc[
                    idx_buscado, "SN CAMBIO"
                ] = serie_seleccionada

                idx_seleccionado = df_averias[
                    df_averias["Serie"].astype(str).str.strip()
                    == serie_seleccionada
                ].index[0]
                st.session_state.df_global_stock.loc[
                    idx_seleccionado, "Entidad"
                ] = match_df.iloc[0]["Entidad"]
                st.session_state.df_global_stock.loc[
                    idx_seleccionado, "Región"
                ] = match_df.iloc[0]["Región"]
                st.session_state.df_global_stock.loc[
                    idx_seleccionado, "Provincia"
                ] = match_df.iloc[0]["Provincia"]
                st.session_state.df_global_stock.loc[
                    idx_seleccionado, "Distrito"
                ] = match_df.iloc[0]["Distrito"]
                st.session_state.df_global_stock.loc[
                    idx_seleccionado, "Tipo estado"
                ] = "Asignado"
                ...
                try:
                  ...
                  if res_json.get("status") == "success":
                    ...
                    st.session_state.mensaje_exito_cambio = (
                        f"⚠️ Actualizado localmente, error en Sheets:"
                        f" {res_json.get('message')}"
                    )
                except Exception as err:
                  st.session_state.mensaje_exito_cambio = (
                      f"⚠️ Actualizado localmente, fallo de red con Sheets:"
                      f" {err}"
                  )
            ...
            st.info(
                f"ℹ️ No se encontraron registros con Part Number"
                f" **{part_number_buscado}** y Entidad **{opcion_cambio}**."
            )
      else:
        st.info(
            f"ℹ️ No se encontró ningún registro asociado a la serie"
            f" **'{serie_a_validar}'** en el sistema."
        )
  ...
  with col_h1:
    st.markdown(
        "<p style='font-size: 1.15rem; font-weight: 700; color: #0f172a; margin:"
        " 0;'>📝 Mantenimiento, Traslados y Bajas de Activos</p>",
        unsafe_allow_html=True,
    )
  with col_h2:
  ...
  if (
      "mensaje_exito_mant" in st.session_state
      and st.session_state.mensaje_exito_mant
  ):
  ...
  if serie_a_editar:
    if df_stock_mant.empty:
      ...
      df_stock_mant["Serie_clean"] = (
          df_stock_mant["Serie"].astype(str).str.strip()
      )
      busq_edit_clean = serie_a_editar.strip()
      match_edit = df_stock_mant[
          df_stock_mant["Serie_clean"].str.lower() == busq_edit_clean.lower()
      ]

      if not match_edit.empty:
        st.info(
            f"✏️ Editando registro para la serie: **{serie_a_editar}**"
            " (Modifique los campos solicitados y guarde)"
        )
        ...
        config_columnas_editor = {
            "Tipo estado": st.column_config.SelectboxColumn(
                "Tipo estado", options=estados_disponibles_lista, required=True
            )
        }
          ...
          try:
            ...
            if res_json.get("status") == "success":
              st.session_state.mensaje_exito_mant = (
                  "✅ ¡Cambios guardados exitosamente en la aplicación y en"
                  " Google Sheets!"
              )
            else:
              st.session_state.mensaje_exito_mant = (
                  f"⚠️ Guardado local, pero Apps Script reportó:"
                  f" {res_json.get('message')}"
              )
          except Exception as err:
            st.session_state.mensaje_exito_mant = (
                f"⚠️ Guardado localmente, pero falló la conexión con Google"
                f" Sheets: {err}"
            )
        ...
        st.warning(
            f"⚠️ No se encontró ningún equipo con la serie"
            f" **'{serie_a_editar}'** en el stock actual."
        )
  ...
  with col_h1:
    st.markdown(
        "<p style='font-size: 1.15rem; font-weight: 700; color: #0f172a; margin:"
        " 0;'>📄 Generador GR — Visor y Editor de Hoja de Cálculo</p>",
        unsafe_allow_html=True,
    )
  with col_h2:
  ...
  embed_url = sheet_url.replace(
      "/edit?gid=0#gid=0", "/edit?embedded=true&gid=0"
  )
  ...
  with col_b2:
    st.markdown(
        f"<div style='text-align: right;'><a href='{sheet_url}'"
        " target='_blank'>Abrir en Google Sheets a pantalla completa"
        " ↗</a></div>",
        unsafe_allow_html=True,
    )
